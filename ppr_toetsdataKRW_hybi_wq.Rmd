...
# remove samplelocations that are not allocated to an EAG
...
# create a unique waarneming (parameter)
...
# add additional names and order of waarneming/ indicators and subindicators
...
EKRset <- merge(EKRset, orderMaatlatten[, c('level','GHPR_level')], by = 'GHPR_level', all.x = TRUE, allow.cartesian = T)
EKRset <- EKRset[GHPR == 'Waterdiepte', GHPR := as.factor(gsub('Waterdiepte',"Vestigingsdiepte waterplanten", GHPR))]
...
# inlezen toetsdata
EKRset <- readRDS(paste0(dirKRW,'EKRsetKRW', toetsjaar,'.rds'))
EKRset2 <- readRDS(paste0(dirKRW,'EKRsetOvWater', toetsjaar, '.rds'))
# goals per EAG/ water body
doelen <- data.table::fread('./input/Doelen.csv')

# add KRW doelen/ goals and addition info of waterbodies
EKRset <- ppr_ekr(krwset = EKRset, ovwatset = EKRset2, eag_wl = eag_wl, doelen = doelen) 
saveRDS(EKRset, file= paste0(dirKRW, "EKRset",".rds"))
EKRset2 <- NULL

## calculate EKR scores per year, average of the last 3 years from EKRset
ekr_scores <- tabelEKRPerWLEnEAGPerJaar(EKRset, detail = "deel")
...
# Einde planperiode 1 = LAATSTE 3 beschikbare meetjaren in periode 2003 tm 2014 (gelijk aan Toestand 2015 factsheets in WKP*)
# Einde planperiode 2 = LAATSET 3 beschikbare meetjaren in periode 2010 tm 2020 (gelijk aan Toestand 2021 factsheets in WKP*)
# Actueel = LAATSTE 3 beschikbare meetjaren in periode 2011 tm 2021 (meeste recente beschikbare gegevens)

ekr_scores_wbp1 <- tabelEKRPerWLEnEAGPerJaar(EKRset[EKRset$jaar > 2005 & EKRset$jaar < 2015,], detail = "deel")
ekr_scores_wbp2 <- tabelEKRPerWLEnEAGPerJaar(EKRset[EKRset$jaar > 2010 & EKRset$jaar < 2021,], detail = "deel")
# aggregated waterbody scores for app
ekr_scores1 <- ekr_scores[!grepl("^NL11_*",id), ] # alleen gewogen scores selecteren
# non aggregated scores for tab per EAG for app
...
trendekr.eag <- merge(ekr_scores, trendekr.eag, by = c('id','EAGIDENT','facet_wrap_code', 'GHPR_level','watertype', "GEP_2022","waterlichaam"))
