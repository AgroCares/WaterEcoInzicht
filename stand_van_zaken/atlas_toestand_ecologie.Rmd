...
setwd("C:/Users/laura/Documents/WaterEcoInzicht/stand_van_zaken")
```
...
gEAG$watertype <- NULL
EAG <- gEAG %>% st_transform(28992)

gKRW <- st_read(paste0("./input/", snapshot, "/gebiedsinfo/WBPKRW20200525.gpkg"))%>% st_transform(4326)
KRW <- gKRW %>% st_transform(28992)
...
eag_wl <- fread(paste0("./input/", snapshot, '/gebiedsinfo/EAG_Opp_kenmerken_20220811.csv'))
...
# data P loads (to do: create and export this data in ppr script)
...
# gp_c <- draw_map(KRW, ekr_scores1[ekr_scores1$level == 1,], bkpl = EAG,
...
suppressMessages(map <- draw_map(pl = KRW, dt = ekr_scores1[ekr_scores1$level == 1,], bkpl = EAG,
                                 ...
                                 col_area_sf = "OWMIDENT", col_area_dt = "KRW_SGBP3",
                                 filename = "./output/map_oordeel_waterlichaam.jpg"))
```
...
include_graphics('./output/map_oordeel_waterlichaam.jpg')
```
...
# krw <-
KRWmapEAG(gEAG, ekr_scores2, maatlat = "2V1 Overige waterflora", param = "oordeel")
...
De ontwikkeling in ecologische kwaliteit kan op verschillende manieren worden verbeeld; De ontwikkeling kan als tijdreeks worden gepresenteerd, als een verschil tussen planperioden en als trend in de tijd.

Voor de jaarlijkse rapportage over de stand van zaken wordt het verschil tussen de kwaliteit van waterplanten in de perioden van het eerste, tweede en derde stroomgebiedbeheerplannen gebruikt. Deze planperioden zijn relevant om de toestand te kunnen relateren aan de maatregelen die in deze perioden zijn uitgevoerd. In de formele KRW-factsheets wordt de eerst gerapporteerde toestand (toestand 2009), de laatst gerapporteerde toestand in de eerste planperiode (toestand 2015), de laatst gerapporteerde toestand in de tweede planperiode (toestand 2021) en de meest recente toestand weergegeven als deze periodeoordelen. Op het moment dat de toestand wordt bepaald en gerapporteerd zijn er nog geen gegevens beschikbaar die zijn verzameld in hetzelfde jaar, daarom zijn de gegevens die worden gebruikt altijd een jaar ouder dan het toetsjaar. Voor de toestand die in 2015 is bepaald is dus gebruik gemaakt data van de laatste drie meetjaren die is verzameld tussen 2006 en 2014. 

In de kaarten hieronder zijn dezelfde perioden gebruikt als voor de formele rapportage van de toestand 2015, 2021 en 2023. In de formele toetsing worden de laatste drie meetjaren gebruikt om te komen tot een periodeoordeel en de gebruikte gegevens mogen maximaal 10 jaar vóór het rapportagejaar zijn verzameld. Hierdoor kunnen de gebruikte data voor verschillende periodeoordelen overlappen en worden verschillen tussen planperioden minder goed zichtbaar. Daarom is er in deze rapportage voor gekozen om de laatste 3 meetjaren in niet overlappende tijdsintervallen te gebruiken voor een periodeoordeel.

```{r verschilkaart1, fig.cap = "Verschil tussen de ecologische kwaliteit van waterplanten sinds de start van de monitoring (gebaseerd op metingen 2006 t/m 2013) en de drie meest recente meetjaren (2021 t/m 2023).", echo = FALSE, message = FALSE, warning = FALSE, out.width='100%'}
# difference between most recent (wbp3) and reference (2006-2013)
wbp03 <- merge(tabsetwbp3[,c('EAGIDENT','KRW_SGBP3','GHPR_level', 'EKR3jr')], tabset, by = c('EAGIDENT','KRW_SGBP3','GHPR_level'))
...
map <- sp::merge(gEAG,gebiedData[,c('param','EKR3jr.x','EKRref','oordeel_2022',
                                    'EAGIDENT','GEP_2022',
                                    'KRWwatertype.code','GHPR_level','EKRperc90.all.year','param')], by.x = 'EAGIDENT', by.y =
                   'EAGIDENT', all.x = TRUE, duplicateGeoms = F)

leaflet() %>%
  addPolygons(data = map, layerId = map$EAGIDENT, 
...
# saveWidget(krw, "verschil1.html", selfcontained = FALSE)
# webshot("verschil1.html", file = "waterflora.png")

```


```{r verschilkaart2, fig.cap = "Verschil tussen de ecologische kwaliteit van de waterplanten aan het einde van de planperiode van SGBP2 (gebaseerd op metingen 2015 t/m 2020) en de drie meest recente meetjaren (2021 t/m 2022).", echo = FALSE, message = FALSE, warning = FALSE, out.width='100%'}

# difference between end wbp2 (2021 - data up 2 2020) and most recent
wbp23 <- merge(tabsetwbp3[,c('EAGIDENT','KRW_SGBP3','GHPR_level', 'EKR3jr')], tabsetwbp2, by = c('EAGIDENT','KRW_SGBP3','GHPR_level'))
...
map <- sp::merge(gEAG,gebiedData[,c('param','EKR3jr.x','EKR3jr.y','EKRref','oordeel_2022',
                                    'EAGIDENT','GEP_2022',
                                    'KRWwatertype.code','GHPR_level','EKRperc90.all.year')], by.x = 'EAGIDENT', by.y =
...
leaflet() %>%
  addPolygons(data = map, layerId = map$EAGIDENT, 
              popup= paste("EAG naam", map$GAFNAAM, "<br>",
                           ...
                           "Doel:", map$GEP_2022, "<br>",
                           "Percentiel90 wbp2:", map$EKRperc90.all.year, "<br>",
                           "Maatlat:", map$GHPR_level ),
              stroke = T, color= 'grey', opacity=0.8, weight = 0.5, smoothFactor = 0.8,
              fill=T, fillColor = ~pal(param), fillOpacity = 0.6) %>%
  addLegend("bottomright", colors = col1, 
            labels= labels1,
            title = 'Verschil toestand 2021 en toestand 2024')%>%
  addProviderTiles("Esri.WorldGrayCanvas")#addTiles()
...
```{r verschilkaart3, fig.cap = "Verschil tussen de ecologische kwaliteit van waterplanten van het einde van de planperiode van SGBP1 (gebaseerd op metingen 2006 t/m 2014) en planperiode SGBP2 (gebaseerd op metingen tussen 2015 t/m 2020).", echo = FALSE, message = FALSE, warning = FALSE, out.width='100%'}

# difference between end wbp2 (2021 - data up 2 2020) and most recent
wbp12 <- merge(tabsetwbp2[,c('EAGIDENT','KRW_SGBP3','GHPR_level', 'EKR3jr')], tabsetwbp1, by = c('EAGIDENT','KRW_SGBP3','GHPR_level'))
wbp12$verschil <- wbp23$EKR3jr.x - wbp23$EKR3jr.y
...
leaflet() %>%
  addPolygons(data = map, layerId = map$EAGIDENT, 
              popup= paste("EAG naam", map$GAFNAAM, "<br>",
                           ...
                           "Doel:", map$GEP_2022, "<br>",
                           "Percentiel90 wbp 1:", map$EKRperc90.all.year, "<br>",
                           "Maatlat:", map$GHPR_level ),
              stroke = T, color= 'grey', opacity=0.8, weight = 0.5, smoothFactor = 0.8,
              fill=T, fillColor = ~pal(param), fillOpacity = 0.6) %>%
  addLegend("bottomright", colors = col1, 
...
```{r add trendindicators for percentage}
trendekreag$trend <- 'kloptniet'
setDT(trendekreag)

trendekreag[p.value < 0.1 &  r.squared < 1 & estimate <= -0.05 & !is.na(estimate), trend := 'negatief']
trendekreag[p.value < 0.1 &  r.squared < 1 & estimate >= 0.05 & !is.na(estimate), trend := 'positief']
trendekreag[p.value < 0.1 &  r.squared < 1 & estimate > -0.05 & estimate < 0.05 & !is.na(trendekreag$estimate), trend := 'neutraal']
trendekreag[(p.value >= 0.1 | r.squared >= 1 | is.na(estimate) | is.na(p.value) | is.na(r.squared)), trend := 'onbekend']

# trend all
round(nrow(trendekreag[trend == 'positief' ])/nrow(trendekreag) * 100, 0)
round(nrow(trendekreag[trend == 'negatief' ])/nrow(trendekreag) * 100, 0)
round(nrow(trendekreag[trend == 'neutraal' ])/nrow(trendekreag) * 100, 0)
round(nrow(trendekreag[trend == 'onbekend' ])/nrow(trendekreag) * 100, 0)

# trend '2V3 Bedekking som submerse planten en draadalgen'
round(nrow(trendekreag[trend == 'positief' & GHPR_level == '2V3 Bedekking som submerse planten en draadalgen'])/nrow(trendekreag[GHPR_level == '2V3 Bedekking som submerse planten en draadalgen']) * 100, 0)
round(nrow(trendekreag[trend == 'negatief' & GHPR_level == '2V3 Bedekking som submerse planten en draadalgen'])/nrow(trendekreag[GHPR_level == '2V3 Bedekking som submerse planten en draadalgen']) * 100, 0)
round(nrow(trendekreag[trend == 'neutraal' & GHPR_level == '2V3 Bedekking som submerse planten en draadalgen'])/nrow(trendekreag[GHPR_level == '2V3 Bedekking som submerse planten en draadalgen']) * 100, 0)
round(nrow(trendekreag[trend == 'onbekend' & GHPR_level == '2V3 Bedekking som submerse planten en draadalgen'])/nrow(trendekreag[GHPR_level == '2V3 Bedekking som submerse planten en draadalgen']) * 100, 0)

# trend "2V21 Soortensamenstelling macrofyten"
round(nrow(trendekreag[trend == 'positief' & GHPR_level == "2V21 Soortensamenstelling macrofyten"])/nrow(trendekreag[GHPR_level == "2V21 Soortensamenstelling macrofyten"]) * 100, 0)
round(nrow(trendekreag[trend == 'negatief' & GHPR_level == "2V21 Soortensamenstelling macrofyten"])/nrow(trendekreag[GHPR_level == "2V21 Soortensamenstelling macrofyten"]) * 100, 0)
round(nrow(trendekreag[trend == 'neutraal' & GHPR_level == "2V21 Soortensamenstelling macrofyten"])/nrow(trendekreag[GHPR_level == "2V21 Soortensamenstelling macrofyten"]) * 100, 0)
round(nrow(trendekreag[trend == 'onbekend' & GHPR_level == "2V21 Soortensamenstelling macrofyten"])/nrow(trendekreag[GHPR_level == "2V21 Soortensamenstelling macrofyten"]) * 100, 0)
...
combineWidgets(
  nrow = 1,
  plottrendEAG(ekrtrendeag, gEAG, maatlat = "2V1 Overige waterflora"),
  plottrendEAG(ekrtrendeag, gEAG, maatlat = "3M1 Macrofauna-kwaliteit")
  
)

combineWidgets(
  nrow = 1,
  plottrendEAG(ekrtrendeag, gEAG, maatlat = "1F1 Fytoplankton-kwaliteit"),
...
# krw <-
#   KRWmapEAG(gEAG,ekr_scores2[ekr_scores2$facet_wrap_code == "Ov. waterflora" ,],
...
p <- ggplot(srtbelas, aes(x= wp_min_sum, y= EKR3jr, label = paste0(waterlichaam), col = KRWwatertype.code.x))+
  geom_jitter() +
...
combineWidgets(
  ...
  plottrendEAG2(trendekreag, gEAG,maatlat = "Soortensamenstelling macrofyten Oeverplanten"),
  plottrendEAG2(trendekreag, gEAG, maatlat = "Soortensamenstelling macrofyten Waterplanten")
)

plottrendEAG(trendekreag, gEAG, maatlat = c("2V22 Abundantie groeivormen macrofyten"))
#krw <-
combineWidgets(
  ncol = 2, colsize = c(1,1),
  plottrendEAG(trendekreag, gEAG, maatlat = c('2V3 Bedekking som submerse planten en draadalgen', "2V3 Vestigingsdiepte waterplanten")),
  plottrendEAG(trendekreag, gEAG, maatlat = c('2V3 Bedekking Emerse planten')),
  plottrendEAG(trendekreag, gEAG, maatlat = c("2V3 Bedekking Kroos")),
  plottrendEAG(trendekreag, gEAG, maatlat = c("2V3 Bedekking Flab (Floating Algae Beds)"))
  )
...
wq1 <- wq[grepl('VAST*', wq$MEETNET_HISTORIE),]

#chlfa<-
combineWidgets(
  ncol = 2, colsize = c(1, 1),
  chlorofylA(wq1[jaar < 2016& wq1$jaar > 2005,], gEAG),
  chlorofylA(wq1[wq1$jaar > '2015'& wq1$jaar < '2025',], gEAG)
)
...
combineWidgets(
  ncol = 3,
  plottrendEAG(trendekreag, gEAG, maatlat = "1F1 Fytoplankton-kwaliteit"),
  plottrendEAG(trendekreag, gEAG, maatlat = "1F2 Abundantie fytoplankton"   ),
  plottrendEAG(trendekreag, gEAG, maatlat = "1F2 Abundantie fytoplankton")
)
...
```{r kaartenekroordeeldoelvis, fig.cap = 'Overzicht van oordelen, EKR scores en doelen op de maatlat vis.', echo = FALSE, message = FALSE, warning = FALSE, out.width="100%", fig.show ='hold'}
...
vislijst <- ekrlijst[aggregated = 'nee',]
...
p<- ggplot(visbelas, aes(x= wp_min_sum, y= biomassa, col = KRWwatertype.code, label = paste0(waterlichaam, jaar.x)))+
  geom_jitter() +
  #facet_grid(~jaar, scales = 'free')+
  theme_minimal()+
  theme(
...
p<- ggplot(visbelasbrkp, aes(x= wp_min_sum, y= biomassa, col = KRWwatertype.code, label = paste0(waterlichaam, jaar.x)))+
  geom_jitter() +
  #facet_grid(~jaar, scales = 'free')+
  theme_minimal()+
  theme(
...
combineWidgets(
  ncol = 4, colsize = c(1,1),
  ekrmapKRW(EKRset[EKRset$jaar >= '2006' & EKRset$jaar < '2010' & EKRset$facet_wrap_code == "Vis",],
            gKRW, maatlat = "4VI1 Vis-kwaliteit" , col=col,labels=labels),
  ekrmapKRW(EKRset[EKRset$jaar >= '2010' & EKRset$jaar < '2013'& EKRset$facet_wrap_code == "Vis",],
            gKRW,   maatlat = "4VI1 Vis-kwaliteit" ,col=col,labels=labels),
  ekrmapKRW(EKRset[EKRset$jaar >= '2013' & EKRset$jaar < '2016'& EKRset$facet_wrap_code == "Vis",], 
            gKRW,   maatlat = "4VI1 Vis-kwaliteit" ,col=col,labels=labels),
  ekrmapKRW(EKRset[EKRset$jaar >= '2016' & EKRset$jaar < '2020'& EKRset$facet_wrap_code == "Vis",], 
            gKRW,  maatlat = "4VI1 Vis-kwaliteit" ,col=col,labels=labels))
...
ekrtrend <- trendekreag[aggregated == 'ja', ]
...
Per (deel)maatlat en ecologisch analysegebied is een lineaire trend in EKR-scores van alle beschikbare meetjaren bepaald. In de figuren met trends in EKR scores worden alleen trends getoond van gebieden waar voor minimaal 2 meetjaren gegevens beschikbaar zijn en waarvan de relevantie voldoende is (meer dan 0.05 ekr per 6 jaar).

Voor de verschillenkaart wordt een verschil tussen twee planperioden van meer dan 0.05 ekr als een lichte voor- of achteruitgang gelabeld. Een verschil groter dan 0.15 is als een grote voor- of achteruitgang gelabeld.
