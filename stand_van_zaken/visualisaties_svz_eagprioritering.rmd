...
```{r plots wateroeverptn indicatiewaarde per soort}
for(i in unique(ekrlijst$EAGIDENT)){
  # i <- unique(ekrlijst$EAGIDENT)[1]
  krw <- ekrlijst[EAGIDENT %in% i,]
  ...
  plot <- annotate_figure(plot, left = text_grob("gemiddeld aantal soorten per meetlocatie", rot = 90, size = 11))
  ggplot2::ggsave(plot,file=paste0('output/',i,'soortenWaterplanten.png'),
...
for(i in unique(ekrlijst$KRW_SGBP3)){
...
```{r plt met ekr scores en doelen}
  ...
  # NL11 is ongewogen
ekr_scores1 <- ekr_scores[aggregated == 'ja', ]
...
wq[,jaar:= year(datum)]
wq[,maand:= month(datum)]
wq[,season := fifelse(maand %in% 4:10,'summer','winter')]

plot <- 
  ggplot(data = wq[parameterid == "Ptot_mgP_l" & locatie %in% c( 'SAP010'),], 
         aes(x=jaar, y=meetwaarde, col= locatie, fill = locatie, group = jaar))+
    geom_point(stat = "summary", fun = "mean")+
    geom_line(stat = "summary", fun = "mean")+   
    #facet_wrap(.~fewsparameter)+
    theme_minimal()+
    ggtitle('')+ ylab('')+ xlab('jaar')+
    theme (axis.text.x =element_text(angle=90, vjust=1))
ggplotly(p=plot)

#  "pH" "SO4_mg_l_nf" "ZS_mg_l" "SO4_mg_l" 
# "CHLFa_ug_l_spectro" "Ca_mg_l" "E__m_L380nm" "Cl_mg_l"
#""Ntot_mgN_l"  NH4_mgN_l_nf" "Ptot_mgP_l" "NKj_mgN_l" "NO3_mgN_l_nf" 
# "MGETAL_mgHCO3_l"

wq <- fychem
wq[parameterid == 'SO4_mg_l_nf', parameterid := 'SO4_mg_l']
wq[parameterid == 'Ca_mg_l_nf', parameterid := 'Ca_mg_l']
wq[parameterid == 'Cl_mg_l_nf', parameterid := 'Cl_mg_l']
wq[parameterid == 'MGETAL_mgHCO3_l_nf', parameterid := 'MGETAL_mgHCO3_l']

# wq1 <- wq[ locatie %in% c( 'OBT006', 'PMW057','PMW072') & jaar > 2010,]
# wq1 <- wq[ EAGIDENT %in% c('3302-EAG-1','3302-EAG-1'),] 

plot <- 
  ggplot(data= wq[parameterid == "ZS_mg_l"   & EAGIDENT %in% c('3302-EAG-1','3302-EAG-1') & jaar > 2015,],
         aes(x=datum, y=meetwaarde, col= locatie, fill = locatie))+
    geom_point()+
    geom_line()+
  # ylim(0,20)+
    #facet_wrap(.~fewsparameter)+
  ...
  ggtitle('Oostelijke binnenpolder van Tienhoven en Taartpunt') +
  labs(x= "datum",y= "Concentratie in mg/l" )
ggplotly(p=plot)
...
# subset gebieden voor prioritering agrarisch
prio <- gEAG[gEAG$prio4 > 0.9,] # methode 1, alleen agrarische EAGs
#stedelijk prio
prio <- gEAG[gEAG$EAGIDENT %in% stedEAG$EAGIDENT,]
#overig prio
...
selb[,Fe_S_PW := FESPPWratio_FeS]
selb[FESPPWratio_FeS > 15,Fe_S_PW := 15]
ggplot(selb, aes(x= Ptot_mgP_l_nf_PW, y= Ptot_mgP_l_ng_SB, col = FESPPWratio ))+
      scale_color_binned(breaks = c(0, 0.5,1,2.5,5,10,15), type = 'viridis')+
      geom_jitter() +
      theme_minimal()

ggplot(selb, aes(x= Ptot_mgP_l_nf_PW, y= Fe_mg_l_nf_PW, col = FESPPWratio_FeS ))+
      scale_color_binned(breaks = c(0, 0.5,1,2.5,5,10,15), type = 'viridis')+
      geom_jitter() +
      theme_minimal()

bodekr <- merge(selb, ekrlijst[aggregated == 'ja',], by = c('EAGIDENT','jaar'))

ggplot(bodekr[GHPR == "Soortensamenstelling macrofyten"], aes(x= Ptot_mgP_l_nf_PW, y= Numeriekewaarde, col = FESPPWratio_FeS ))+
      scale_color_binned(breaks = c(0, 0.5,1,2.5,5,10,15), type = 'viridis')+
      geom_jitter() +
      theme_minimal()
...
set.seed(42)
cols_num <- colnames(selb)[sapply(selb, is.numeric)]
selb_n <- selb[,c(cols_num,'EAGIDENT'), with=FALSE]
sample_split <- sample.split(Y = selb_n$Ptot_mgP_l_nf_PW, SplitRatio = 0.7)
train_set <- subset(x = selb_n, sample_split == TRUE)
test_set <- subset(x = selb_n, sample_split == FALSE)

y_train <- as.integer(train_set$EAGIDENT) - 1
y_test <- as.integer(test_set$EAGIDENT) - 1
X_train <- train_set %>% select(-Ptot_mgP_l_nf_PW)
X_test <- test_set %>% select(-Ptot_mgP_l_nf_PW)
