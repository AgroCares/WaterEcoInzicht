...
eag_wl <- data.table::fread(paste0('./input/', workspace, '/gebiedsinfo/EAG_Opp_kenmerken_20220811.csv'))
...
setDT(brpAGV)
brpAGV <- brpAGV[,sum(as.numeric(agropp)), by = EAGIDENT]
# merge relative agricultural
eag_wl <- merge(eag_wl, brpAGV, by.x = 'GAFIDENT', by.y = 'EAGIDENT', all.x = TRUE)
eag_wl[,relagr := V1/OppLand]
eag_wl[relagr > 0.5 & V1 > 100000, gebiedstype := 'agrarisch']
...
fychem[,maand:= month(datum)]
wq.sel <- fychem[,.(locatie,parameterid,meetwaarde,jaar,maand, EAGIDENT, KRW_SGBP3, compartiment, watertype, FEWSFILTER_ACTUEEL)]
...
wq.sel$fewsparameter <- gsub('_nf','',wq.sel$parameterid) 
# selectie relevant parameters
wq.par <- c('Ptot_mgP_l','PO4_mgP_l',"NO3_mgN_l","Ntot_mgN_l",'NH4_mgN_l',"ZICHT_m","Cl_mg_l" ,
           'O2_mg_l','pH','T_oC','SO4_mg_l',"SO4_mg_l","CHLFa_ug_l_spectro",
           "CHLFa_ug_l_blauwalg" , "Ca_mg_l",
           "Ca_mg_l",'MGETAL_mgHCO3_l',"MGETAL_mgHCO3_l_nf","Mg_mg_l","Mg_mg_l_nf","ZS_mg_l" )
wq.sel <- wq.sel[parameterid %in% wq.par]
wq.sel.wide <- dcast( wq.sel,KRW_SGBP3+EAGIDENT+locatie+watertype+parameterid+season+FEWSFILTER_ACTUEEL~jaar, value.var = 'meetwaarde', fun.aggregate = mean, fill = FALSE)
