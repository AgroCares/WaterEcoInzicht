...
plot <- ggplot(data = wq[parameterid == "CHLFa_ug_l_spectro" & locatie %in% c( 'BOT001','BOT002'),], aes(x=jaar, y=meetwaarde, col= locatie, fill = locatie, group = jaar))+
...
wq <- fychem
plot <- ggplot(data= wq[parameterid == "Ptot_mgP_l" & locatie %in% c( 'PKH020') & jaar >2015,], aes(x=datum, y=meetwaarde, col= locatie, fill = locatie))+
    geom_point()+
    geom_line()+
    #facet_wrap(.~fewsparameter)+
    theme_minimal()+
    ggtitle('')+ ylab("")+ xlab('datum')+
    theme (axis.text.x =element_text(angle=90, vjust=1))
ggplotly(p=plot)
