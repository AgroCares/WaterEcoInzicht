...
# inladen basis bestanden ----
  ...
  eag_wl <- data.table::fread('data/EAG_Opp_kenmerken_20221003.csv')
...
# data voor EST tekening ----------------------
  ...
  EST_kt <- data.table::fread('data/EST_koppeltabel.csv')
 
  # show progress
  pbc <- pbc + 1; setTxtProgressBar(pb,pbc)
  
# bewerk ESF oordeel op basis van data--------------------
  
  #add status ESF as text
  #ESF1
  ESFoordelen$ESF1 <- ifelse(ESFoordelen$ESF1_nr == '3', paste('Het watersysteem voldoet niet aan de eisen van de sleutelfactor productiviteit water. 
                                                               De hoeveelheid voedingsstoffen die dit watersysteem binnenkomt is hoger dan de draagkracht van het ecosysteem waardoor kroos of algen gaan woekeren.
                                                               Te veel kroos of algen belemmeren de groei van ondergedoken waterplanten.', ESFoordelen$ESF1),
                             ifelse(ESFoordelen$ESF1_nr == '2',paste('Het watersysteem voldoet mogelijk of lokaal niet aan de eisen van de sleutelfactor productiviteit water. 
                                                               De hoeveelheid voedingsstoffen die dit watersysteem binnenkomt is mogelijk of lokaal hoger dan de draagkracht van het ecosysteem waardoor kroos of algen gaan woekeren.
                                                               Te veel kroos of algen belemmeren de groei van ondergedoken waterplanten.', ESFoordelen$ESF1),
                                    ifelse(ESFoordelen$ESF1_nr == '1',paste('Het watersysteem voldoet aan de eisen van de sleutelfactor productiviteit water. 
                                                               De hoeveelheid voedingsstoffen die dit watersysteem binnenkomt is lager dan de draagkracht van het ecosysteem. 
                                                                            Kroos of algen zijn geen belemmering voor de groei van ondergedoken waterplanten', ESFoordelen$ESF1),
                                           paste('Het is onbekend of het watersysteem voldoet aan de eisen van de sleutelfactor productiviteit water. 
                                                               Het is niet bekend of de hoeveelheid voedingsstoffen die dit watersysteem binnenkomt hoger of lager is dan de draagkracht van het ecosysteem 
                                                               en woekerende kroos of algen een belemmering vormen voor de groei van ondergedoken waterplanten', ESFoordelen$ESF1))))
...
# --- extractfunctie for relevant properties needed for single factsheet ----
  ...
  factsheetExtract <- function(i,brondata,splot = TRUE){ with(brondata, {
    ...
    # get hydrobiological data
    EST_sel <- EST[locatie.EAG %in% eagwl$GAFIDENT,]
    ...
    if(nrow(EST_sel)>0){
      ...
      cols <- colnames(EST_sel)[grepl('^locatie.EAG|jaar|^O|^W|SGBP3|GAFNAAM|KRW',colnames(EST_sel))]
      dtEST2 <- merge.data.table(EST_sel[,mget(cols)], eagwl[,c('GAFIDENT','watertype','StedelijkLandelijk')], by.x = "locatie.EAG", by.y  = "GAFIDENT")
      ...
      namenest <- colnames(dtEST4[,3:20])
      dtEST4 <- dtEST4[,lapply(.SD, function(x) mean(as.numeric(x))),.SDcols = 3:20]
      ...
      dtEST4$W <- namenest[dtEST4$W]

      # make selection name for esticons
      ESTnaam1 <- paste0(c('','O'),c(dtEST4[,.(W,O)]),collapse = '_')
      ESTnaam2 <- unique(dtEST2$watertype)
      ESTnaam2 <- ifelse(ESTnaam2 == 'M20','DM',
                         ifelse(ESTnaam2 %in% c('M14','M27',"M25","M11"),'OM',
                                ifelse(ESTnaam2 %in% c('M1a','M1b','M8',"M10","M3"),'Sl','K')))
      ESTnaam3 <- ifelse(unique(dtEST2$StedelijkLandelijk) == 'Stedelijk','St','L')

      # final ESTnaam
      ESTnaam <- paste0(ESTnaam1,'_',ESTnaam2,'_', ESTnaam3)
    ...
    # percentage per type
    periode <- c("SGBP1 2009-2015", "SGBP2 2015-2021", "SGBP3 2021-2027") # hier mis een periode (niet in sgbp)
    rates <- list(
    ...
    m_gauges <- data.table(title = c('Maatregelen SGBP 1 en 2\n uitgevoerd of in uitvoering t.o.v. totaal',
                                     'Maatregelen SGBP 1 en 2\n in planvorming t.o.v. totaal',
                                     'Maatregelen SGBP 1 en 2\n gefaseerd t.o.v. totaal',
                                     'Maatregelen SGBP 1 en 2\n vervangen of ingetrokken t.o.v. totaal',
                                     'Maatregelen SGBP3\n nieuw t.o.v. totaal'),
