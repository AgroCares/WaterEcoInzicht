...
ppr_ekr <- function(krwset, ovwatset, eag_wl, doelen){
  ...
  # check <- unique(db$HoortBijGeoobject.identificatie[is.na(db$GEP_2022) & db$level == 1])
...
ppr_wq <- function(db,syear = NULL,wtype = NULL,mlocs = NULL, srow= c("IONEN*|NUTRI*|ALG*|VELD*|ALGEN*|LICHT*|MONSVAR*")){
  ...
  db[,c('EAGIDENT','watertype','OWMIDENT','GAFIDENT') := NULL]
  ...
  db <- db[grepl(srow, fewsparametercategorie, fixed = FALSE)]
...
ppr_wbalfiles <- function(dir_bal,EAG.sf = gEAG,kopTab = kopTab){
  ...
  bal_mis <- kopTab[!balans %in% files, balans]
  kop_mis <- as.data.frame(files[!files %in% kopTab$balans])
  # write.csv2(kop_mis, file = "KOPMIS.CSV")
...
ppr_esf_oordeel <- function(dir = 'input/toestand_esf/'){
...
ppr_tabelPerWL3jaargemEAG_incl2022 <- function (EKRset){
  ...
  d1 <- d1[,lastyear := max(jaar),by = colg]
  ...
  d1 <- d1[,.(EKR = mean(waarde,na.rm=T)), by = c(colg,'lastyear')]
...
ppr_plotbod <- function(bod1, type='grid'){
  ...
  # dcast slootbodem
  selb <- dcast.data.table(bod1, `locatie EAG`+locatiecode+`locatie omschrijving`+`locatie x`+`locatie y`+`locatie z`+datum+jaar ~ fewsparameter+compartiment, value.var = "meetwaarde", fun.aggregate = mean)
  ...
  if(is.null(selb$Stot_mg_l_nf_PW) & !is.null(selb$Stot_mg_l_PW)){
  ...
  if(is.null(selb$FESPPWratio)){selb[,FESPPWratio := NaN]}
  ...
  plotFW <- ggplot(selb, aes(x= `locatie EAG`, y= nlvrFW, fill = classFESPFWratio))+
    geom_boxplot() +
    theme_minimal()+
    theme(
      strip.background = element_blank(),
  ...
  if(!is.null(selb$FESPPWratio)){
    qPW <- ggplot(selb, aes(x= `locatie EAG`, y= nlvrPW, fill = classFESPPWratio))+
      geom_boxplot() +
      theme_minimal()+
      theme(
        strip.background = element_blank(),
...
# Vanaf hier komen functies die niet voor de KRW factsheets worden gebruikt -----------
# ekr per eag en mp
