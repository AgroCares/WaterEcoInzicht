...
plotChangeAW <- function(l){
  l <- l[l$Typering.code %in% c('FYTOPL','OVWFLORA',"MAFAUNA",'VIS'),] #alleen hoofdmaatlatten
...
plotEKRlijnToetsgebied <- function(z, detail = "hoofd"){
  ...
  z<- z[is.na(z$Monster.lokaalID)|z$Monster.lokaalID == "",] # alleen scores per meetlocatie per jaar
  if(detail == "hoofd"){
    z<- z[z$Typering.code %in% c('FYTOPL','OVWFLORA',"MAFAUNA",'VIS'),] #alleen hoofdmaatlatten
  }
  z$waterlichaam <- sapply(strsplit(z$HoortBijGeoobject.identificatie, '_'), `[`, 2)
  z$waterlichaam[is.na(z$waterlichaam)] <- paste0('gewogen_',z$HoortBijGeoobject.identificatie[is.na(z$waterlichaam)])
  z$waterlichaam[z$waterlichaam == 'OvWa'] <- sapply(strsplit(z$HoortBijGeoobject.identificatie[z$waterlichaam == 'OvWa'], '_'), `[`, 3)
...
plotEKRlijnfs <- function(z, gebied = NULL){
  ...
  ggplot(data= z, aes(x=jaar, y=Numeriekewaarde, col = GHPR, group = GHPR))+
    ...
    stat_summary(fun = "mean", geom = "line") +
    scale_y_continuous(limits= c(0, 1), breaks=c(0, 0.2, 0.4, 0.6, 0.8, 1))+
    scale_x_continuous(limits= c(2006, 2020), breaks=c(2006, 2008, 2010, 2012, 2014,2016,2018,2020,2022))+
...
tabelOordeelPerGebiedPerJaar <- function (b, detail = "hoofd"){
  ...
  d1 <- copy(EKRset)
  if(detail == "hoofd"){
  d1 <-  d1[d1$Typering.code %in% c('FYTOPL','OVWFLORA',"MAFAUNA",'VIS'),]}
...
ekrmap_mpeag <- function(EKRset, gEAG, maatlat = "2V1 Overige waterflora", col=col, labels=labels){
    ...
    groups <- c('EAGIDENT','HoortBijGeoobject.identificatie','waterlichaam','KRWwatertype.code','KRW_SGBP3','GHPR_level','GHPR','level','Waardebepalingsmethode.code') 
