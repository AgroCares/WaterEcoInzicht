...
`r out$waterlichamenwl$Karakterschets` <br>
`r out$waterlichamenwl$OWMNAAM_SGBP3`
(`r out$waterlichamenwl$OWL_SGBP3`) heeft watertype
"`r unique(out$waterlichamenwl$typebesch)`"
(`r unique(out$eagwl$watertype)`) en het wateroppervlak van het
waterlichaam is `r round(sum(out$eagwl$OppWater)/10000, 0)` hectare.
<br> Het waterlichaam bestaat uit de deelgebieden: <br>
`r out$deelgebieden$samen` <br> Het waterlichaam ligt in de provincie(s)
`r out$waterlichamenwl$prov` en gemeente(n) `r out$waterlichamenwl$gem`.
...
Het waterlichaam `r out$waterlichamenwl$OWMNAAM_SGBP3` heeft de status
`r out$waterlichamenwl$Status` en is in eigendom van
`r out$waterlichamenwl$Eigenaren`.
...
**De huidige toestand vergeleken met de doelen
--**`r unique(out$d3$oordeel_2022)` <br> De toestand in
`r out$waterlichamenwl$OWMNAAM_SGBP3` (zwarte lijnen in de figuur
hiernaast) is `r unique(out$d3$oordeel_2022)`. Het biologische
kwaliteitselement met het laagste oordeel is
`r as.character(unique(out$d3$facet_wrap_code))`.
`r if(!unique(out$d3$facet_wrap_code) %in% "Macrofauna"){unique(paste0("De slechtst scorende deelmaatlat van dit kwaliteitselement is ",unique(out$d3_deel$GHPR),"."))}`
`r if(!unique(out$d3$facet_wrap_code) %in% "Macrofauna"){if(unique(out$d3_deel$GHPR) == 'Abundantie groeivormen macrofyten'){unique(paste0("De slechtst scorende indicator van deze deelmaatlat is ", out$d3_deelptn$GHPR,"."))}}`
...
Deze factsheet is gebaseerd op de KRW-toetsing (aan maatlatten 2018) uit
`r out$toetsjaar`, begrenzing waterlichamen 2022-2027, hydrobiologische
data `r out$hybiversie`, conceptmaatregelen en doelen voor SGBP3
`r if(!is.na(out$waterlichamenwl$Literatuurverwijzing)|!out$waterlichamenwl$Literatuurverwijzing == ''){paste0(" en ",out$waterlichamenwl$Literatuurverwijzing)}`.
Het beeld (tekening) van de toestand van het waterlichaam is gemaakt
door Annemoon van Steen.
...
### SGBP 1 en 2 maatregelen die (deels) zijn uitgevoerd t.o.v. totaal
...
### SGBP 1 en 2 maatregelen in planvorming t.o.v. totaal
...
### SGBP 1 en 2 maatregelen die zijn gefaseerd t.o.v. totaal
...
### SGBP 1 en 2 maatregelen die zijn ingetrokken of vervangen t.o.v. totaal
...
`r out$waterlichamenwl$motstat` \### Monitoringswensen
...
**Waterlichaam** De waterlichamen vormen de basisrapportageeenheden van
