...
# load info on EAG: names and relation with KRW (for the future: retrieve names from EAG map and relation KRW from separate table)
...
eag_wl[,GAFIDENT := sapply(strsplit(EAGIDENT, '-'), `[`, 1)]
eag_wl[,EAGNAAM := GAFNAAM]
eag_wl[,GAFNAAM := sapply(strsplit(EAGNAAM, ','), `[`, 1)]
...
locaties <- merge(locaties, eag_wl[,c('EAGIDENT','GAFNAAM','KRW_SGBP3','watertype')], by = 'EAGIDENT', all.x = TRUE, all.y =FALSE)
...
# filter only eags were watertype is correct
ekrlijst <- ekrlijst[paste0(EAGIDENT,KRWwatertype.code) %in% unique(paste0(EAG$EAGIDENT,EAG$OWMTYPE))]
# Botshol 2011 waar zijn monsters eag 2 gebleven?
...
setnames(EAG,c("GAFIDENT" ,"Naam","Soort","Opp_EAG","Omschr" ,"Opmerking" , "Wtype_BR"  , "Wtype_Vast",  "geometry"),
...
# merge EAG with KRW
EAG <- merge(EAG, eag_wl[, c('EAGIDENT','KRW_SGBP3','KRWmonitoringslocatie_SGBP3','SGBP3_NAAM')], by = 'EAGIDENT', all.x = TRUE)
...
# post process chemical data (take only relevant categories, which differs for each type of analysis) 4 use in atlas_toestand
fychem <- ppr_wqdata(fychem, syear = 1900, srow = c("HYBIO","IONEN","NUTRI","ALG","VELD","ALGEN","LICHT","REST","MONSVAR","METAL","NUTRI_KRWTOETS","ALG_KRWTOETS","ALGEN_KRWTOETS","HYBIO_KRWTOETS","KRWTOETS","IONEN_KRWTOETST", "VELD_KRWTOETST"))
