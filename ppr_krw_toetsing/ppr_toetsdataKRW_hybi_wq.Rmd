...
locaties <- merge(locaties, eag_wl[,c('EAGIDENT','GAFNAAM','KRW_SGBP3','watertype')], by.x = 'EAGIDENT', by.y = 'EAGIDENT', all.x = TRUE)
...
fychem <- import_wqdata(path = paste0('./input/', snapshot, '/fychem/'), location = locaties, parameter = parameter)
# post process chemical data (take only relevant categories, which differs for each type of analysis) 4 use in atlas_toestand
...
```{r impor bodchem}
#import chemical data
bodchem <- import_wqdata(path = paste0('./input/', snapshot, '/bodemchemie/'), location = locaties, parameter = parameter)
