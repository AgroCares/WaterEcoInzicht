...
ekrtrend <- readRDS('./data/krw/ekrtrend.rds') %>% as.data.table() #alleen hoofd, gewogen
ekrtrendeag <- readRDS('./data/krw/ekrtrendeag.rds')  %>% as.data.table() #alleen hoofdmaatlat
# write.table(trendekreag, file = paste(getwd(),"/output/trendekreag",format(Sys.time(),"%Y%m%d%H%M"),".csv", sep= ""), quote = FALSE, na = "", sep =';', row.names = FALSE)

EKRlijst <- readRDS('./data/krw/EKRlijstKRW.rds')
...
maatlat <- "2V1 Overige waterflora"
  ...
  map <- sp::merge(gEAG,gebiedData[,c('param','EKR3jr','EKR3jr.x','EKRref','oordeel','oordeel_2022',
                                      'EAGIDENT','GEP','GEP_2022','EKRperc90.allejaren',
                                      'watertype','GHPR_level')], by.x = 'GAFIDENT', by.y =
  ...
  gebiedData2$verschil <- gebiedData2$`2021` - gebiedData2$EKRref
  ...
  map3 <- sp::merge(gEAG, gebiedData2[,c('param','2021','EKR3jr','EKRref','oordeel','oordeel_2022',
                                         'EAGIDENT','GEP','GEP_2022','EKRperc90.allejaren', 'watertype',
                                            'GHPR_level')], by.x = 'GAFIDENT', by.y =
                         'EAGIDENT', all.x = TRUE, duplicateGeoms = T)
  map3<- map3[!is.na(map3$`2021`),]
  sf_use_s2(FALSE)
  mapcenter <- st_centroid(map3)
  pal2 <- colorFactor(palette = col1,  domain = map3$param)


  leaflet() %>%
    addPolygons(data = map, layerId = map$GAFIDENT, popup= paste("EAG naam", map$GAFNAAM, "<br>",
                                                                "EKR score wbp1:", map$EKR3jr.x, "<br>",
    ...
    addCircles(data= mapcenter,  popup =  paste("EKR score:", mapcenter$`2021`,"<br>",
                                                "Meetjaar: 2021"),
                 weight = 8, radius=40, fillOpacity = 0.8, color= ~pal2(param))%>%
    addLegend("bottomright", colors=col1, labels=labels1, title = 'Kleur gebieden: verschil WBP2 en WBP1')%>%
    addLegend("bottomright", colors=col1, labels=labels1, title = 'Stippen: EKR scores 2021')%>%
