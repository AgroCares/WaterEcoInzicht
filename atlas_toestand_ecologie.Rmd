...
subtitle: "Een verzameling visualisaties van de ontwikkelingen in ecologische waterkwaliteit in het beheergebied van waterschap Amstel, Gooi en Vecht."
author: "Laura Moria"
date: "`r Sys.Date()`"
...
# Inleiding
De huidige situatie en ontwikkelingen van het aquatisch ecosysteem in de regio Amstel, Gooi en Vechtstreek is in deze rapportage gebundeld in een atlas met thema kaarten. De ecologische doelstellingen van Amstel, Gooi en Vecht, de huidige ecologische kwaliteit en de ontwikkeling in de tijd zijn verbeeld in de kaarten hieronder.  

De KRW toestandsbepaling dient watersysteemanalyses, die ten behoeve KRW gebiedsprocessen, operationele besturing en de evaluatie van maatregelen worden uitgevoerd. Daarnaast wordt de ecologische toestand per waterlichaam (of deelgebied) gerapporteerd in de KRW factsheets (www.waterkwaliteitsportaal.nl) en het waterbeheerplan van AGV. 
...
De ontwikkeling in ecologische kwaliteit kan op verschillende manieren worden verbeeld. Voor de jaarlijkse rapportage over de stand van zaken wordt het verschil tussen het eerste en tweede stroomgebiedbeheerplan weergegeven. En daarbovenop de metingen van het meest recente, nog niet eerder gerapporteerde meetjaar.

```{r verschilkaart, fig.cap = "Verschil in ecologische kwaliteit tussen WBP 1 en 2", echo = FALSE, message = FALSE, warning = FALSE, out.width='100%'}

# make this into a function 
wbp12 <- merge(tabsetwbp1[,c('EAGIDENT','id','GHPR_level', 'EKR3jr')], tabsetwbp2, by = c('EAGIDENT','id','GHPR_level'))
wbp12$verschil <- wbp12$EKR3jr.y - wbp12$EKR3jr.x
...
maatlat <- "2V1 Overige waterflora"
  gebiedData <- wbp12[!is.na(wbp12$EAGIDENT),]
  gebiedData <- gebiedData[gebiedData$GHPR_level %in% maatlat,]

  gebiedData <- gebiedData[!is.na(gebiedData$verschil),]
  gebiedData$EKR3jr <- gebiedData$EKR3jr.y
  gebiedData <- gebiedData[!is.na(gebiedData$verschil),]
    '7' -> gebiedData$klasse[gebiedData$verschil < -0.2]
    '6' -> gebiedData$klasse[gebiedData$verschil >= -0.2 & gebiedData$verschil < -0.1]
    '5' -> gebiedData$klasse[gebiedData$verschil >= -0.1 & gebiedData$verschil < 0]
    '4' -> gebiedData$klasse[gebiedData$verschil >= 0 & gebiedData$verschil < 0.1]
    '3' -> gebiedData$klasse[gebiedData$verschil >= 0.1 & gebiedData$verschil < 0.2]
    '2' -> gebiedData$klasse[gebiedData$verschil >= 0.2]
    gebiedData$param <- as.factor(gebiedData$klasse)
    gebiedData$param = factor(gebiedData$param, levels = c("2","3", "4", "5", "6","7"))
    col1 <- c('2'="darkgreen", '3'="chartreuse",'4'= 'palegreen', '5'="darksalmon",'6'="red",'7'= 'darkred')
    labels1 <- c('2'=">0.2",'3'="0.2 - 0.1",'4'="0.1 - 0",'5'="0 - -0.1",'6'="-0.1 - -0.2" ,'7'="< -0.2")
    gebiedData <- gebiedData[!is.na(gebiedData$klasse),]

  pal <- colorFactor(palette = col1,  domain = gebiedData$param)
  map <- sp::merge(gEAG,gebiedData[,c('param','EKR3jr','EKR3jr.x','EKRref','oordeel','oordeel_2022',
                                      'EAGIDENT','GEP','GEP_2022',
                                      'watertype','GHPR_level','EKRperc90')], by.x = 'GAFIDENT', by.y =
                     'EAGIDENT', all.x = TRUE, duplicateGeoms = T)
  
  # voeg centerpoint met recente score toe
  lastyear <- as.character(max(ekr_scores2$lastyear))
  gebiedData2 <- ekr_scores2[!is.na(ekr_scores2$EAGIDENT),]
  gebiedData2 <- gebiedData2[gebiedData2$GHPR_level %in% maatlat,]
  gebiedData2$lastyear <- gebiedData2[,..lastyear]
  gebiedData2$verschil <- gebiedData2$lastyear - gebiedData2$EKRref
  
  '7' -> gebiedData2$klasse[gebiedData2$verschil < -0.2]
  '6' -> gebiedData2$klasse[gebiedData2$verschil >= -0.2 & gebiedData2$verschil < -0.1]
  '5' -> gebiedData2$klasse[gebiedData2$verschil >= -0.1 & gebiedData2$verschil < 0]
  '4' -> gebiedData2$klasse[gebiedData2$verschil >= 0 & gebiedData2$verschil < 0.1]
  '3' -> gebiedData2$klasse[gebiedData2$verschil >= 0.1 & gebiedData2$verschil < 0.2]
  '2' -> gebiedData2$klasse[gebiedData2$verschil >= 0.2]
  gebiedData2$param <- as.factor(gebiedData2$klasse)
  gebiedData2$param = factor(gebiedData2$param, levels = c("2","3", "4", "5", "6","7"))
  gebiedData2 <- gebiedData2[!is.na(gebiedData2$klasse),]
  
  cols <- c('param','EKR3jr','EKRref','oordeel','oordeel_2022','EAGIDENT','GEP','GEP_2022',
            'watertype','GHPR_level','EKRperc90','lastyear')
  map3 <- sp::merge(gEAG, gebiedData2[,..cols], by.x = 'GAFIDENT', by.y =
                         'EAGIDENT', all.x = TRUE)
  map3 <- map3[!is.na(map3$param),]
  
  suppressMessages(sf_use_s2(FALSE))
  mapcenter <- st_centroid(map3)
  
  pal2 <- colorFactor(palette = col1,  domain = mapcenter$param)


  leaflet() %>%
    addPolygons(data = map, layerId = map$GAFIDENT, 
                popup= paste("EAG naam", map$GAFNAAM, "<br>","EKR score wbp1:", map$EKR3jr.x, "<br>",
                             "EKR score wbp2:", map$EKR3jr, "<br>",
    ...
    addCircles(data= mapcenter,  popup =  paste("EKR score:",mapcenter$lastyear,"<br>",
                                                paste0("Meetjaar: ", lastyear)),
                 weight = 8, radius=40, fillOpacity = 0.8, color= ~pal2(param))%>%
    addLegend("bottomright", colors=col1, labels=labels1, 
              title = 'Kleur gebieden: verschil WBP2 en WBP1')%>%
    addLegend("bottomright", colors=col1, labels=labels1, 
              title = paste0('Stippen: EKR scores ', lastyear))%>%
...
#krw <-
  combineWidgets(
    ncol = 3,
    plottrendEAG(trendekreag, gEAG, maatlat = c('Bedekking som submerse planten en draadalgen', 'Vestigingsdiepte waterplanten')),
...
## Overzichtstabel met scores, oordelen en ontwikkeling

```{r toestandtrend, fig.cap = "Toestand, oordeel en ontwikkeling", echo = FALSE, message = FALSE, warning= FALSE, out.width='100%'}
